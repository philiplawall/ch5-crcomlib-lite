...
export class Ch5Signal<T extends TSignal> {
  ...
  private static _resetEventsInitialization() {
    ...
    const processResyncronizationCallback = (
      resyncRequest: IResynchronizationRequestModel
    ) => {
      ...
      // CLEAR RANGE
        ...
        resyncRequest.state === ResetEventNames.startOfUpdateRange ||
        resyncRequest.state === ResetEventNames.startOfUpdateRangeSO
      ) {
        if (
          resyncRequest.value !== undefined &&
          ...
          ch5Resync.onReceiveStartOfUpdateRange(
            resyncRequest.state,
            resyncRequest.value.range,
            resyncRequest.value.excludePrefixes
          );
        } else {
          throw new Error("Invalid resyncRequest object");
        }
    ...
    subscribeState(
      ...
      processResyncronizationCallback
    );
  }
  ...
  constructor(name: string, typeInstance: T) {
    ...
    this._subject = new Ch5SignalBehaviorSubject<T>(
      this._uninitializedValue,
      this._uninitializedValue
    );
    this._lastSubId = 0;
    this._subscriptions = {};
  ...
  public fromSignalBridge(value: T) {
    // for use only for signals received from native on signal bridge
    ...
    if (this._subject.getValue() !== value) {
      ...
      // even though the component does not need to be alerted of change, resync logic needs to know join was received.
      this._ch5Resync.onReceiveUpdatedState(this._name, this.value, this.type);
    }
  ...
  public subscribe(
    updatecb: Ch5SignalUpdateCallback<T>,
    errorcb?: Ch5SignalErrorCallback
  ): string {
    this._lastSubId++; // add one to last subscription
    const subKey: string =
      this._name + "-" + ("00000" + this._lastSubId).slice(-5); // pad zeros
    ...
    if (Object.keys(this._subscriptions).length === 1) {
      ...
      this._signalBridge.subscribe(
        this._name,
        typeof this._uninitializedValue as TSignalStandardTypeName
      );
    }
    return subKey;
  }
  ...
  public unsubscribe(subKey: string) {
    ...
    if (oldSub !== undefined) {
      ...
      if (Object.keys(this._subscriptions).length === 1) {
        ...
        this._signalBridge.unsubscribe(
          this._name,
          typeof this._uninitializedValue as TSignalStandardTypeName
        );
      }
