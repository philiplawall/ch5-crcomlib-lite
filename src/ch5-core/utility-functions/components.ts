...
export function countNumberOfCh5Components(parentElement: any) {
  ...
  for (const element of allElements) {
    ...
    if (elementTagName.startsWith('CH5-')) {
      ...
      priorFoundForTagName === undefined
        ? (found[elementTagName] = 1)
        : (found[elementTagName] = priorFoundForTagName + 1)
    } else {
      // not starts with 'ch5-'
      if (element.hasAttributes()) {
        ...
        for (let idx = attrs.length - 1; idx >= 0; idx--) {
          ...
          if (
            attrName.startsWith('data-ch5') &&
            attrName !== 'data-ch5-id' &&
            !attrName.endsWith('subs-key') &&
            !attrName.endsWith('sub-key')
          ) {
            found.total++
