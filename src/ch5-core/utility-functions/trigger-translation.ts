...
} from "../ch5-translation-factory";
import { Ch5SignalFactory } from "../ch5-signal-factory";

export function triggerTranslation(language?: string) {
  const languageChangedSig = Ch5SignalFactory.getInstance().getStringSignal(
    languageChangedSignalName
  );
  let changedLanguage: string | undefined =
    translationFactory.translator.language;

  if (
    (changedLanguage === undefined || changedLanguage === null) &&
    language !== ""
  ) {
