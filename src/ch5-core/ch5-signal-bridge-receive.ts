...
export function bridgeReceiveIntegerFromNative(
  signalName: string,
  value: number
): void {
...
export function bridgeReceiveBooleanFromNative(
  signalName: string,
  value: boolean
): void {
  // Check for join number signal state name
...
export function bridgeReceiveStringFromNative(
  signalName: string,
  value: string
): void {
  // Check for join number signal state name
...
export function bridgeReceiveObjectFromNative(
  signalName: string,
  value: object
): void {
  // Check for join number signal state name
  ...
  if (Ch5Debug.shouldDisplay(dbgKey)) {
    Ch5Debug.info(
      dbgKey,
      '"' + _signalName + "\":'" + JSON.stringify(value) + "'"
    );
  }
 ...
 */
function processReceivedRcb(
  signalName: string,
  rcbObj: Ch5RcbSimpleObject,
  currentScalarValue: number
): Ch5RcbExtendedObject {
  ...
  if (RCB_INTERVAL_DURATION_MS < rcbObj.rcb.time) {
    const intervalId = window.setInterval(
      rcbIntervalTimerCallback,
      RCB_INTERVAL_DURATION_MS,
      signalName,
      rcbObject
    );
 ...
 * number signal
 */
function rcbIntervalTimerCallback(
  signalName: string,
  rcbObject: Ch5RcbExtendedObject
): void {
  ...
  if (rcbObject.rcb.time === 0) {
    // safeguard against undefined slope
    return;
  }
   ...
   */
  const slope =
    (rcbObject.rcb.value - rcbObject.rcb.startv) / rcbObject.rcb.time;
