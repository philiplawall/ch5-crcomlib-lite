...
export class Ch5TranslationUtility {
  ...
  public valuesToTranslation(valueToTranslation: string) {
    const patternIdentifier = new RegExp(
      "(\\" +
        this.translatorBeginKey +
        ")([a-zA-Z0-9\\.]+)\\w+(\\" +
        this.translatorEndKey +
  ...
  public hasMultipleIdentifiers(valueToTranslation: string) {
    ...
    if (
      valueToTranslation.match(patternIdentifier) &&
      (valueToTranslation.match(patternIdentifier) as RegExpExecArray).length >
        1
    ) {
      return true;
    }
  ...
  public stripDownTranslationCharacters(value: string) {
    ...
    if (
      pattern.exec(value) &&
      (pattern.exec(value) as RegExpExecArray).length > 0
    ) {
      translatedCaracters = (
        pattern.exec(value) as RegExpExecArray
      )[2] as string;
    }
