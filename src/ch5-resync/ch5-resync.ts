...
export class Ch5Resync {
   ...
   * @param state
   */
  public static checkIfStateShouldBeIncluded(
    excludeStatesWithThesePrefixes: ICh5ExcludePrefixesModel,
    state: string
  ) {
   ...
   * @memberof Ch5Resync
   */
  public onReceiveStartOfUpdate(
    excludeStatesWithThesePrefixes: ICh5ExcludePrefixesModel
  ): void {
    Ch5Debug.info(
      this.LOG_KEY,
      `Start of Update called with ${JSON.stringify(
        excludeStatesWithThesePrefixes
      )}`
    );
   ...
   */
  public onReceiveStartOfUpdateRange(
    startOfUpdateMsg: string,
    statesToReset: ICh5ClearRangeDataModel,
    excludePrefixes: string[]
  ): void {
    if (Ch5Debug.shouldDisplay(this.LOG_KEY)) {
      Ch5Debug.info(
        this.LOG_KEY,
        `Start of Update Range called with ${JSON.stringify(statesToReset)}`
      );
    }
    ...
    this.addDefaultJoinsOnClearRange(
      statesToReset.boolean.joinLow,
      statesToReset.boolean.joinHigh,
      "boolean"
    );
    this.addDefaultJoinsOnClearRange(
      statesToReset.numeric.joinLow,
      statesToReset.numeric.joinHigh,
      "number"
    );
    this.addDefaultJoinsOnClearRange(
      statesToReset.string.joinLow,
      statesToReset.string.joinHigh,
      "string"
    );

    // Add events to _statesAtDefaultValue
    this.addDefaultStatesOnClearRange(
      statesToReset.boolean.stateNames,
      "boolean",
      excludePrefixes
    );
    this.addDefaultStatesOnClearRange(
      statesToReset.numeric.stateNames,
      "number",
      excludePrefixes
    );
    this.addDefaultStatesOnClearRange(
      statesToReset.string.stateNames,
      "string",
      excludePrefixes
    );
    ...
    if (Ch5Debug.shouldDisplay(this.LOG_KEY)) {
      Ch5Debug.info(
        this.LOG_KEY,
        `Start Of Update Range:${JSON.stringify(this._statesAtDefaultValue)}`
      );
    }
  }
  ...
  public onReceiveUpdatedState(stateName: string, value: any, type: any): void {
    ...
    if (
      this._statesAtDefaultValue[type] !== undefined &&
      this._statesAtDefaultValue[type][stateName] !== undefined
    ) {
  ...
  public onReceiveEndOfUpdate(): void {
    Ch5Debug.info(
      this.LOG_KEY,
      `End of Update called with counter ${this.startOfUpdateCounter}`
    );
   ...
   * @param eventType
   */
  public addDefaultJoinsOnClearRange(
    joinLow: number,
    joinHigh: number,
    eventType: string
  ): void {
    ...
    for (let i = joinLow; i <= joinHigh; i++) {
      this._statesAtDefaultValue[eventType][
        Ch5ResyncConstants.JOIN_NUMBER_SIGNAL_NAME_PREFIX + i.toString()
      ] = true;
    }
  }
   ...
   * @param eventType
   */
  public addDefaultStatesOnClearRange(
    states: string[],
    eventType: string,
    excludePrefixes: string[]
  ): void {
   ...
   * @memberof Ch5Resync
   */
  public setDefaultStatesOnStartOfUpdate(
    excludeStatesWithThesePrefixes: ICh5ExcludePrefixesModel
  ): void {
    this._statesAtDefaultValue = {};
      ...
      if (this.statesRef.hasOwnProperty(key)) {
        ...
        for (const state in this.statesRef[key]) {
          if (this.statesRef[key].hasOwnProperty(state)) {
            ...
            if (
              Ch5Resync.checkIfStateShouldBeIncluded(
                excludeStatesWithThesePrefixes,
                currentState[state]._name
              ) &&
    ...
    if (Ch5Debug.shouldDisplay(this.LOG_KEY)) {
      Ch5Debug.info(
        this.LOG_KEY,
        `Start of Update:${JSON.stringify(this._statesAtDefaultValue)}`
      );
    }
  }
  ...
  public setRemainingStatesToDefaultValue(): void {
    if (Ch5Debug.shouldDisplay(this.LOG_KEY)) {
      Ch5Debug.info(
        this.LOG_KEY,
        `End Of Update:${JSON.stringify(this._statesAtDefaultValue)}`
      );
    }
              ...
              switch (stateType) {
                ...
                case "number":
                  ...
                  if (this.statesRef.object[stateName]) {
                    this.statesRef.object[stateName].fromSignalBridge({
                      rcb: { value: 0, time: 0 },
                    });
                  }
                  break;
  ...
  private initializeStartOfUpdateTimer(
    time: number = Ch5Resync.START_OF_UPDATE_TIME_LIMIT
  ) {
    window.clearTimeout(this.startOfUpdateTimer);
    this.startOfUpdateTimer = window.setTimeout(() => {
      Ch5Debug.info(
        this.LOG_KEY,
        `Timeout waiting for EndOfUpdate remainingEOUs:${this.startOfUpdateCounter}`
      );
      this.resetRemainingStates();
